...
2.1. *Kiến trúc hệ thống: MVVM + GetX*
	...
	**View Layer (screens/)**: Chịu trách nhiệm hiển thị giao diện người dùng và bắt các tương tác, sử dụng GetBuilder hoặc Obx của GetX để tự động cập nhật UI.

	**ViewModel Layer (viewmodels/)**: Cầu nối giữa View và Controller, chứa logic trình bày dữ liệu và quản lý trạng thái UI như validate form, quản lý trạng thái loading.

	**Controller Layer (controllers/)**: Chứa logic nghiệp vụ cốt lõi, điều phối hoạt động giữa các Service, xử lý dữ liệu thô và thực hiện quy tắc nghiệp vụ.

	**Service Layer (services/)**: Truy cập dữ liệu và tương tác với hệ thống bên ngoài như API, Firebase, hoặc phần cứng (camera, microphone) theo Repository Pattern.

	**Model Layer (models/)**: Định nghĩa cấu trúc dữ liệu với các lớp Dart thuần túy chứa thuộc tính và phương thức chuyển đổi từ/sang JSON.
